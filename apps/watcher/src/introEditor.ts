...
import ws from 'ws';
import { buildIntroSpec, type IntroSpec } from './intro';
import { probeVideo } from './video';
import { createLogger } from './logger';
import { EpisodeStore } from './supabase';
import { autoCaptionSrt as autoCaption } from './captions';

const log = createLogger('intro-editor');
...
function applyParams(p){
  ...
  // Rebuild from the pause so the layout is fully pause-relative (clicks/voiceover
  // anchor to T; grain/boom to the end; music spans all). Matches the watcher.
  var audio=[
...
// Stored in Supabase so the dashboard channel form + watcher can use the same
// presets. Set from startIntroEditor (the watcher passes its store; standalone
// builds one from env). Null = no Supabase configured → preset endpoints no-op.
let presetStore: EpisodeStore | null = null;

function storeFromEnv(): EpisodeStore | null {
  const url = process.env.SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !serviceRoleKey) {
    log.warn('Presets need SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY — preset save/load disabled.');
    return null;
  }
  // @supabase/realtime-js needs a global WebSocket at client creation (Node < 22).
  if (typeof (globalThis as { WebSocket?: unknown }).WebSocket === 'undefined') {
    (globalThis as { WebSocket?: unknown }).WebSocket = ws;
  }
  return new EpisodeStore({ url, serviceRoleKey });
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/presets') {
    ...
    presetStore
      .listIntroPresets()
      .then((presets) => res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify(presets)))
      .catch((e) => res.writeHead(500).end(e instanceof Error ? e.message : 'Failed to list presets'));
    return;
  }
  if (req.method === 'POST' && req.url === '/presets') {
    if (!presetStore) {
      res.writeHead(503).end('Presets need SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY set on the editor.');
      return;
    }
    readJson(req)
      .then(async (body) => {
        const b = body as { name?: string; channel?: string; params?: Record<string, unknown> };
        if (!b.name || !b.params) {
          res.writeHead(400).end('name and params are required');
          return;
        }
        const preset = await presetStore!.createIntroPreset({ name: b.name, channel: b.channel ?? '', params: b.params });
        res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify(preset));
      })
      .catch((e) => res.writeHead(400).end(e instanceof Error ? e.message : 'Invalid JSON'));
    return;
  }
  if (req.method === 'DELETE' && req.url && req.url.startsWith('/presets/')) {
    ...
    presetStore
      .deleteIntroPreset(id)
      .then(() => res.writeHead(200, { 'Content-Type': 'application/json' }).end('{"ok":true}'))
      .catch((e) => res.writeHead(500).end(e instanceof Error ? e.message : 'Failed to delete preset'));
    return;
  }
...
/**
 * Start the intro editor HTTP server. Called standalone (scripts/test-video-ui.ts)
 * for local dev, and from the watcher daemon when INTRO_EDITOR is enabled so it
 * runs in-process on the box that has ffmpeg — no separate service to launch.
 */
export function startIntroEditor(opts?: { port?: number; host?: string; store?: EpisodeStore }): http.Server {
  const port = opts?.port ?? PORT;
  const host = opts?.host ?? HOST;
  // Use the watcher's store if given, else build one from env (standalone dev).
  presetStore = opts?.store ?? storeFromEnv();
  server.listen(port, host, () => {
    log.info(`Intro editor at http://${host}:${port}  (ffmpeg required on PATH)`);
  });
  return server;
}
