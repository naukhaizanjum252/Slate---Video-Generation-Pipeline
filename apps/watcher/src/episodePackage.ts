 ...
 */
export function parseBodyPlan(bundleRoot: string, fullVoDurationSec: number): BodyPlan {
  ...
  // 4. Body VO duration = body share of the full narration. The intro line plays first.
  const introChars = narrationLen(introMd);
  const bodyDurationSec = fullVoDurationSec * (totalBodyChars / (introChars + totalBodyChars));
  const bodyStartSec = fullVoDurationSec - bodyDurationSec;
  ...
  // 6. CTA windows: from each CTA marker until the next marker (its narration block).
